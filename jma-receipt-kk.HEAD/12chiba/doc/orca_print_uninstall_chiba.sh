...
echomsg "千葉県(総括表・地方公費)アンインストール中..."

cp ${SYSCONFDIR}/kentan.inc ${SYSCONFDIR}/kentan.inc.bak
sed '/chiba/d' ${SYSCONFDIR}/kentan.inc.bak > ${SYSCONFDIR}/kentan.inc
...
if test -f ${SITESRCDIR}/data/KOUZA.INI ; then
  rm -rf ${SITESRCDIR}/data/KOUZA.INI
fi
...
if test -f ${SITEDATADIR}/KOUZA.INI ; then
  rm -rf ${SITEDATADIR}/KOUZA.INI 
fi
...
echomsg "千葉県(総括表・地方公費)アンインストール終了！！"
