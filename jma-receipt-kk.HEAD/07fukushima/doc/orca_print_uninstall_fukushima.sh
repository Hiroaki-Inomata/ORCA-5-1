...
echomsg "福島県(総括表・地方公費)アンインストール中..."

cp ${SYSCONFDIR}/kentan.inc ${SYSCONFDIR}/kentan.inc.bak
sed '/fukushima/d' ${SYSCONFDIR}/kentan.inc.bak > ${SYSCONFDIR}/kentan.inc
...
if test -f ${SITESRCDIR}/data/CITYNUM.INI ; then
  rm -rf ${SITESRCDIR}/data/CITYNUM.INI
fi
...
if test -f ${SITEDATADIR}/CITYNUM.INI ; then
  rm -rf ${SITEDATADIR}/CITYNUM.INI 
fi
...
echomsg "福島県(総括表・地方公費)アンインストール終了！！"
