...
sed '/kagoshima/d' ${SYSCONFDIR}/kentan.inc.bak > ${SYSCONFDIR}/kentan.inc
rm -rf ${SYSCONFDIR}/kentan.inc.bak
echo -e "\tkagoshima;" >> ${SYSCONFDIR}/kentan.inc

# site-upgrade.sh run
if [ -f ${SCRIPTSDIR}/allways/site-upgrade.sh ] ; then
	sh ${SCRIPTSDIR}/allways/site-upgrade.sh
else
	echomsg "鹿児島県地方公費プログラムコピー異常終了!!"
	exit
fi

sync
echomsg "鹿児島県地方公費・総括表プログラムコピー終了!!"
