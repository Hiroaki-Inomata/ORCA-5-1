...
echomsg "宮崎県(総括表・地方公費)アンインストール中..."

cp ${SYSCONFDIR}/kentan.inc ${SYSCONFDIR}/kentan.inc.bak
sed '/miyazaki/d' ${SYSCONFDIR}/kentan.inc.bak > ${SYSCONFDIR}/kentan.inc
...
if test -f ${SITESRCDIR}/data/RECESYUTURYOKU.INI ; then
  rm -rf ${SITESRCDIR}/data/RECESYUTURYOKU.INI
fi
...
if test -f ${SITEDATADIR}/RECESYUTURYOKU.INI ; then
  rm -rf ${SITEDATADIR}/RECESYUTURYOKU.INI 
fi
...
echomsg "宮崎県(総括表・地方公費)アンインストール終了！！"
