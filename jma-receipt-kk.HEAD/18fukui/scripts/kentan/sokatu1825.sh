...
PREFNAME=fukui
PROGRAMID=SOKATU1825
LOG_FILE="/var/log/jma-receipt/${15}sokatu1825"
RENNUM=0
#-------------------------------------------#
#    広域連合・診療報酬請求内訳書（福井）
#        $1-${11}
...
##      返戻分（特別療養費）
        if  [ ${19} -eq '0' ] || [ ${19} -eq '2' ]; then
            RENNUM=$(expr $RENNUM + 1) 
            $DBSTUB -dir $LDDIRECTORY -bd $PREFNAME $PROGRAMID -parameter $1,$2,$3,$RENNUM,$5,$6,$7,$8,$9,${10},${11},${15},${12},${13},${18},2,2,${20},${21},${20},${21},${16}
            if  [ -e ${MCP_TEMPDIR}/${16} ]; then
                exit
            fi
        fi
