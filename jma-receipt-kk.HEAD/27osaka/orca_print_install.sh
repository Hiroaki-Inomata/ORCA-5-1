...
echomsg "大阪府（総括表・地方公費）プログラムコピー中..."
...
sed '/osaka/d' ${SYSCONFDIR}/kentan.inc.bak > ${SYSCONFDIR}/kentan.inc
rm -rf ${SYSCONFDIR}/kentan.inc.bak
echo -e "\tosaka;" >> ${SYSCONFDIR}/kentan.inc

# site-upgrade.sh run
if [ -f ${SCRIPTSDIR}/allways/site-upgrade.sh ] ; then
	sh ${SCRIPTSDIR}/allways/site-upgrade.sh
else
	echomsg "大阪府（総括表・地方公費）プログラムコピー異常終了!!"
	exit
fi

sync
echomsg "大阪府（総括表・地方公費）プログラムコピー終了!!"
